...
import com.cookieshax.coursehelper.feature.checkin.model.GestureCheckInStrategy
...
fun GestureTrigger(
    ...
    Box(
        ...
        Button(
            modifier = Modifier.fillMaxWidth(),
            enabled = !isCheckingIn && selectedIds.isNotEmpty() && isCorrectGesture,
            onClick = {
                scope.launch {
                    viewModel.performCheckIn(
                        strategy = GestureCheckInStrategy(),
                        params = CheckInParams.Gesture(
                            url = url,
                            taskId = taskId,
                            courseId = courseId,
                            gestureCode = gesturePoints.joinToString("") { (it + 1).toString() }
                        ),
            ...
            if (isCheckingIn) {
                ...
                Text(text = if (isCorrectGesture) "对选中账号签到" else "请先绘制正确手势")
            }
        }
    }
}
